...
void print_solution(Task *sorted_tasks, int n_tasks, Platform *selected_devs, double time, double energy)
{
	...
	for (i = 0; i < n_tasks; i++)
	{
		n_devs = selected_devs[sorted_tasks[i].id].n_devices;
		printf("     T%d: {", sorted_tasks[i].id);
		for (j = 0; j < n_devs; j++)
		{
			printf("%d(%d", selected_devs[sorted_tasks[i].id].devices[j].id, selected_devs[sorted_tasks[i].id].devices[j].n_cores);
			ht = selected_devs[sorted_tasks[i].id].devices[j].hyperthreading;
			if (ht == 1)
			{
				printf("x2)");
			}
